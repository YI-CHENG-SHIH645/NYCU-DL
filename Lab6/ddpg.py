...
class ActorNet(nn.Module):
    def __init__(self, state_dim=8, action_dim=2, hidden_dim=(400, 300)):
        ...
        self.fc_out = nn.Linear(h2, action_dim)
...
class DDPG:
    def __init__(self, args):
        ...
        self._actor_net.apply(init_weights)
        self._critic_net = CriticNet().to(args.device)
        self._critic_net.apply(init_weights)
        # target network
        self._target_actor_net = ActorNet().to(args.device)
        self._target_critic_net = CriticNet().to(args.device)
        # initialize target network, 100% behavior net
        self._update_target_network(self._target_actor_net, self._actor_net, 1.)
        self._update_target_network(self._target_critic_net, self._critic_net, 1.)
        # TODO #
    ...
    def select_action(self, state, noise=True):
        ...
        action = self._actor_net(state)[0].detach().cpu().numpy()
        return action if not noise else action + self._action_noise.sample()
    ...
    def _update_behavior_network(self, gamma):
        ...
        # critic loss
        # TODO #
        q_value = critic_net(state, action)  # (batch, 1)
        with torch.no_grad():
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                print(
                    ...
                    f'Ewma reward: {ewma_reward:.2f}\t'
                )
                break
    env.close()


def show_video(env, agent):
    def sim():
        done = False
        env.seed(316)
        state = env.reset()
...
def main():
    ...
    parser.add_argument('--show_video_only', action='store_true')
    ...
    if args.show_video_only:
        agent.load(args.model)
        show_video(env, agent)
    else:
