...
class ReplayMemory:
    ...
    def sample(self, batch_size, device):
        ...
        # (batch of state, batch of action, ...)
...
class DQN:
    def __init__(self, args):
        ...
        # initialize target network, 兩個網路架構一樣
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        if random.random() < epsilon:
            ...
            with torch.no_grad():
                ...
                action = self._behavior_net(state).max(1)[1].item()
        return action
    ...
    def update(self, total_steps):
        if total_steps % self.freq == 0:
            ...
            # print(list(self._behavior_net.parameters())[0])
    ...
    def _update_behavior_network(self, gamma):
        ...
        # TODO #
        q_val = self._behavior_net(state).gather(1, action.long())
        q_next = self._target_net(next_state).max(1)[0].detach().view(-1, 1)
    ...
    def _update_target_network(self):
        ...
        # TODO #
        for tgt_params, local_params in zip(self._target_net.parameters(),
                                            self._behavior_net.parameters()):
            tgt_params.data.copy_(local_params.data)
...
def train(args, env, agent, writer):
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            # 隨機選動作，純粹累積 buffer，沒有要更新 policy
            if total_steps < args.warmup:
                action = action_space.sample()  # int
            else:
                # epsilon greedy select & epsilon decay
